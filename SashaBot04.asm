info: SashaBot03, Sasha 
// changed how far to move in one firection before changing direction
// also, changed such that the nanorg always releases half of its energy
// also, changed such that the nanorg pokes and tries to corrupt a drone
...
noFood:                       // see if we're over a collection point and release some energy
        energy  r8            // places the organism’s current energy value into the dest 
                              // register or memory location Flags: No effect on flags
        div     r8, 2         // reduce value by half   
        cmp     r8, 2000      
        ...
        release r8            // drain half of my energy, but get points, assuming
...
notEnufEnergy:                // move me
        ...
        jns     tryPoke       // bumped into another org or the wall
                              // jumps to the address if the SUCCESS flag is NOT set
        sub     [count], 1    // sub dest, src // dest = dest – src
        jmp     loop

tryPoke:                      // try to poke in order to potentially corrupt drones
        mov     r0, 0x401E    // place the code for the SENSE command into r0
        poke    [dir], 9
        mov     r0, 0         // place the code for the SENSE command into r0
        poke    [dir], 7
